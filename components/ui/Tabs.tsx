...
function TabsList({
  ...
  return (
    <TabsPrimitive.List
      className={cn(
        'h-12 items-center justify-center rounded-md bg-gray-100 py-1 px-1.5',
        className
      )}
      {...props}
    />
...
function TabsTrigger({
  ...
  return (
    <TextClassContext.Provider
      value={cn(
        'text-sm native:text-base font-medium text-gray-800 web:transition-all',
        value === props.value && 'text-black font-bold'
      )}
    >
      <TabsPrimitive.Trigger
        className={cn(
          ...
          props.value === value && 'bg-white shadow-md shadow-black/10',
          className
        )}
        {...props}
      />
