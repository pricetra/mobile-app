  ...
  ({ className, value, ...props }, ref) => {
    return (
      <Animated.View className="overflow-hidden" layout={LinearTransition.duration(200)}>
        <AccordionPrimitive.Item ref={ref} className={cn('mb-5', className)} value={value} {...props} />
      </Animated.View>
    );
  }
);
    ...
    <TextClassContext.Provider value="native:text-lg font-medium web:group-hover:underline">
      <AccordionPrimitive.Header className="flex">
        <AccordionPrimitive.Trigger ref={ref} {...props} asChild>
          <Trigger
            className={cn(
              'group flex flex-row items-center justify-between px-5 py-3 web:flex-1 web:transition-all web:focus-visible:outline-none web:focus-visible:ring-1 web:focus-visible:ring-muted-foreground',
              className
            )}>
            <>{children}</>
  ...
  return (
    <TextClassContext.Provider value="native:text-lg">
      <AccordionPrimitive.Content
        ...
        ref={ref}
        {...props}>
        <InnerContent className={cn('px-5 py-3', className)}>{children}</InnerContent>
      </AccordionPrimitive.Content>
    </TextClassContext.Provider>
  );
});
